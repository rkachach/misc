...
def format_table(table):
    table.field_names = ["branch", "description", "merged"]
...
def show_branches(prefix, main_branch):
    ...
    for branch in branches.splitlines():
        ...
        if curr_branch:
            table.add_row([G+branch_name, desc, bmerged+N])
        elif merged:
            table.add_row([Y+branch_name, desc, bmerged+N])
        else:
            table.add_row([N+branch_name+N, desc, bmerged+N])
